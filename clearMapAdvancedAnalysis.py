...
This script was provided by Laura DeNardo and performs analysis as in DeNardo et al., Nature Neuroscience 2019 (https://doi.org/10.1038/s41593-018-0318-7)
...
baseDirectory = '/d2/studies/ClearMap/IA_iDISCO/AdvancedAnalysis/'
trap = pd.read_excel('/d2/studies/ClearMap/IA_iDISCO/analysisFiles/counts_table.xlsx',header=0) #import whole brain cell counts
...
plots_dir = '/d2/studies/ClearMap/IA_iDISCO/AdvancedAnalysis/' 
import os
from sklearn.manifold import TSNE

var = All_Vol
varName = 'Combined_VolumeNormalized'
clu = cluster_snn(var)
...
cluster_areas.to_excel(os.path.join(baseDirectory, varName + '_clusters.xlsx'))

#Plot tSNE:
perplexity=50
ts = TSNE(perplexity=perplexity, random_state=0).fit_transform(np.corrcoef(var)) #select perplexity that makes the most sense for your data
...
plt.savefig(os.path.join(baseDirectory, varName + "_tSNE.png"), bbox_inches='tight')

#Run PCA & Correlations
fos = trap
